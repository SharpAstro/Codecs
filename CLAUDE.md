...
| TIFF **tiled write** | Magick.NET (libtiff) | Just a NuGet reference. `TiffWriterTiledOracleTests` has libtiff read back what `TiffWriter` emits in `TiffLayout.Tiled`. Nothing to install, nothing to skip. **It has to be an outside reader**: `TiffReader` handles strips only, so this repo cannot read its own tiled output. |
