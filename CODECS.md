...
- 🔜 **Gain-map JPEG (Ultra HDR / ISO 21496-1)** — `SharpAstro.Jpeg.GainMap`, an `IccInjector`-style metadata-domain package: read/reconstruct HDR from the SDR base + gain map (both plain baseline JPEGs our decoder already handles), assemble/generate on the write side — the publishing tier for HDR masters (one JPEG, correct on SDR browsers, HDR on capable displays). The gain map is the author-shipped answer to the tone-mapping question the facade deliberately refuses to answer itself. Detailed plan + oracle strategy (libultrahdr): [`ROADMAP-gain-map.md`](ROADMAP-gain-map.md).
