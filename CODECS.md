...
| [`SharpAstro.Jbig2`](src/SharpAstro.Jbig2/) | JBIG2 | — | Pure-managed **clean-room** JBIG2 (ITU-T T.88) bilevel decoder, written for PDF's `/JBIG2Decode` filter — spec-derived, because every reference implementation is licensed in a way an Unlicense repo can't vendor (`jbig2dec` is AGPL; pdf.js / PDFium / `jbig2enc` are notice-retaining). **The primary entry point is not the facade**: `Jbig2Decoder.Decode(embedded, globals, width, height)`. A PDF-embedded stream is not a file — T.88's embedded organization has no file header to sniff, its shared segment dictionaries live in a separate `/DecodeParms /JBIG2Globals` stream, and its dimensions come from the image dictionary, so all three are arguments. Standalone `.jb2` files (sequential **and** random-access organization, striped pages via end-of-stripe) go through `DecodeFile` and register with the facade by magic bytes as a courtesy. **Shipped in 3.7:** the MQ arithmetic decoder (Annex E), generic region decoding **both ways** — arithmetic (GBTEMPLATE 0–3, TPGDON typical prediction, arbitrary AT pixels) and **MMR / ITU-T T.6** (§6.2.6, the Group 4 fax coding, which is also what PDF's `/CCITTFaxDecode` carries at `K < 0`) — page information, and region composition (OR/AND/XOR/XNOR/REPLACE, clipped to the page). **Not yet:** symbol dictionary + text region, generic refinement, halftone — each throws `NotSupportedException` naming the missing feature rather than returning a plausible-looking blank page. **Validation:** the MQ coder is pinned to the published ITU-T T.88 Annex H.2 conformance vector **in both directions** (256 known decisions ⟷ the 30-byte codestream), and each context template is pinned cell-by-cell by one-hot tests against Figures 4–7 — the layer a round-trip can't validate, since an encoder sharing the decoder's context code would share its mistakes. MMR has no round-trip available at all (encoding is a non-goal), so it is checked entirely against foreign bytes: libtiff writes Group 4 via Magick.NET and the decoder has to reproduce the raster, with a sweep that covers **every run length T.4 Tables 2–3 define** — added after a deliberately mislabelled table entry survived the picture-shaped cases. Output is one byte per pixel, **1 = black (T.88 polarity)**, plus an 8-bit grey projection; `/Decode` inversion and `ImageMask` semantics stay in the caller's PDF layer. Remaining rungs: [`ROADMAP-pdf-codecs.md`](ROADMAP-pdf-codecs.md). |
...
- ✅ **`SharpAstro.Jbig2` shipped (3.7), rungs 1–2 of 5** — the MQ arithmetic decoder + generic region decoding in **both** codings (arithmetic templates 0–3 with TPGDON and arbitrary AT pixels, and MMR / ITU-T T.6), page information, and region composition, decoding a meaningful share of real PDFs on its own. Clean-room from ITU-T T.88 and T.4/T.6, with the Annex H.2 conformance vector pinning the arithmetic coder in both directions and libtiff-produced Group 4 bytes pinning MMR. The contract prediction in the roadmap held: it ships an explicit `Jbig2Decoder.Decode(embedded, globals, w, h)` entry point, with `IImageDecoder` registration as a courtesy for standalone `.jb2` files only. Remaining rungs — symbol dictionary + text region, refinement, halftone — plus the external `jbig2dec` oracle harness: [`ROADMAP-pdf-codecs.md`](ROADMAP-pdf-codecs.md).
