...
| [`SharpAstro.Tiff`](src/SharpAstro.Tiff/) | TIFF | TIFF | Full pure-managed TIFF reader/writer. Multi-page, 8/16/32-bit uint + IEEE-Float, Uncompressed / Deflate / Zlib / **LZW** (MSB-first, early-change widths), **Predictor 2** (horizontal differencing) on read, II + MM byte order, SampleFormat/SMin/SMax/ICC round-trip. Predictor 3 (floating-point) is refused rather than decoded wrongly. **Streaming read** (`ReadInto` + `ITiffStripSink`) delivers a page strip by strip so a caller never needs the whole raster; an uncompressed page needing no normalisation is handed over as a slice of the input, so reading one from a memory-mapped file copies nothing. |
