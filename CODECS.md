...
- ✅ **`SharpAstro.Jbig2` shipped (3.7), rung 1 of 5** — the MQ arithmetic decoder + generic region decoding (GBTEMPLATE 0–3, TPGDON, arbitrary AT pixels), page information, and region composition, decoding a meaningful share of real PDFs on its own. Clean-room from ITU-T T.88, with the Annex H.2 conformance vector pinning the arithmetic coder in both directions. The contract prediction in the roadmap held: it ships an explicit `Jbig2Decoder.Decode(embedded, globals, w, h)` entry point, with `IImageDecoder` registration as a courtesy for standalone `.jb2` files only. Remaining rungs — MMR (T.6), symbol dictionary + text region, refinement, halftone — plus the external `jbig2dec` oracle harness: [`ROADMAP-pdf-codecs.md`](ROADMAP-pdf-codecs.md).
