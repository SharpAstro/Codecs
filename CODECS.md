...
| [`SharpAstro.Jxr`](src/SharpAstro.Jxr/) | JXR | JXR | Clean-room T.832 JPEG XR codec from the spec PDF. BD8/BD16/BD16F/BD32F × grayscale/RGB × spatial/frequency mode × multi-tile × INDEX_TABLE_TILES (with random-access tile decode) × POT × lossy quantization × alpha plane × full `.jxr` file container. Optional `useYUV444: true` switches RGB to YCoCg-R + InternalClrFmt=YUV444 for Windows Photo / WIC interop. |
