...
- 📋 **PDF-embedded codecs (JBIG2 / JPX) assessed, not started** — the last two image filters in the PDF spec the family can't decode. **JBIG2** is the recommended next codec (staged: generic region → MMR → symbol dictionary + text region → refinement → halftone), but it does **not** fit `IImageDecoder`: PDF's embedded-stream organization has no file header and needs out-of-band `/JBIG2Globals` + dimensions, so it needs an explicit `Jbig2Decoder.Decode(embedded, globals, w, h)` entry point. **JPX** fits the facade fine (JP2/J2K both have magic bytes) but is a `SharpAstro.Jxr`-scale project (8–15k LOC decode-only) and is gated behind JBIG2. Both must be **clean-room from ITU-T T.88 / T.800** — `jbig2dec` is AGPL and OpenJPEG/PDFium/pdf.js are notice-retaining, so none can be a port source for an Unlicense repo, though all are fine as oracle *binaries*. Full scoping, licence matrix, and validation plan: [`ROADMAP-pdf-codecs.md`](ROADMAP-pdf-codecs.md).
