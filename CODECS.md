...
| [`SharpAstro.Jxl`](src/SharpAstro.Jxl/) | JXL | JXL | Pure-managed **clean-room** JPEG XL (ISO/IEC 18181) — spec-as-judge + Magick.NET (libjxl) as the empirical oracle + jxl-oxide as a read-only bit-layout reference. **Lossless Modular** path: 8/16-bit integer **and IEEE-float (F16/F32)**, grey + RGB, single group (each dimension ≤ 1024); integer RGB decorrelated with the reversible YCoCg-R colour transform. `JxlImageCodec` façade — `EncodeRgb24`/`Gray8`/`Rgb48`/`Gray16` for integer, `EncodeGrayF32`/`GrayF16`/`RgbF16`/`RgbF32` (+ matching `Decode*`) for HDR float (values verbatim, not normalised), `EncodeRgb24Lossy` for **lossy VarDCT** (8-bit RGB, dims multiples of 8 up to 16384), plus a general `Decode` (auto-detects Modular vs VarDCT) and `JxlFile.ReadInfo`. **Validated both directions** — our decode of real libjxl images, and libjxl/Magick decode of our output (pixel-exact for integer; bit-exact self round-trip + value-exact-vs-libjxl for float). Full hybrid-integer / ANS / prefix entropy stack, MA decision tree, 14 predictors (incl. the self-correcting weighted predictor), and RCT/Palette transforms on decode. **Lossy VarDCT** is supported for 8-bit RGB at a fixed high-quality setting (DCT8, XYB, full-resolution chroma, multi-group / multi-LF-group up to 16384 px) — libjxl-validated both directions. A tunable quality/distance knob, grayscale-lossy, alpha, and `do_ycbcr` are not yet supported. |
